...
function pickQuality(type: string, quality: any): number {
...
function baseHeaders(ref: string): any {
    return {
        "User-Agent": UA,
...
async function getSanityKey(timeout: number = 20000) {
...
function toForm(data: any): URLSearchParams {
    const p = new URLSearchParams();
    for (const [k, v] of Object.entries(data)) p.set(k, String(v));
    return p;
}

function normalizeObj(data: any): any {
...
export async function y2mateDirect(url: string, opts: any = {}): Promise<any> {
